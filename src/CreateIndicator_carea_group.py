...
param = {"name":"carea", "longname":"crop area for crop group", "unit":"ha/pixel", "expression":"harea*5000 for all crops", "searchStr":"_carea_sea_", "replaceStr":"_carea_sea_"}
groups = fao_utilities.GetCropGroupsAndFilenames()
for i in range(1, 12):
    items = [group[1] for group in groups if group[0] == '1']  # get the search string of the files to match, e.g. ['_carea_sea_trri_ir', '_carea_sea_trri_rf']
    ...
    for climateModel in ["_can_", "_gfd_", "_mir_"]:  # filter for the files containing only the climate model
        for scenario in ["_B1_", "_B2_"]:  # filter for the files containing only the scenario model
            ...
            if len(filteredFiles) > 0:
                ...
                f = filteredFiles[0][0]
                outputNetCDF = fao_utilities.OUTPUT_DATA + os.path.basename(f)[:os.path.basename(f).find("carea_sea_") + 10] + "gr" + str(i).zfill(2) + ".nc"  # get the path for the new netcdf file
