...
def numpyToImage(numpyfile,imagefile):
    ...
    minx=min(data3d[1]) #get the minx value
    maxx=max(data3d[1]) #get the maxx value
    miny=min(data3d[0]) #get the miny value
    maxy=max(data3d[0]) #get the maxy value
...
#    arcpy.AddMessage("width:" + str(width) + " height:" + str(height))
    data3d[1].__isub__(minx) #change the x values to be zero based by subtracting the minx value
    data3d[0].__imul__(-1)   #do the same with the y values using a different calculation
    data3d[0].__iadd__(maxy)
    pixels=coo_matrix((data3d[2],(data3d[0],data3d[1])), shape=(height,width)).todense() #convert the sparse array into a dense matrix, i.e. by adding in all of the zero values
